...
export const DisplayBondPrice = ({ bond }: { bond: IAllBondData }): ReactElement => {
  ...
  if (typeof bond.bondPrice === "undefined" || !bond.isAvailable[NETWORK_CHAINID]) {
...
export const DisplayBondDiscount = ({ bond }: { bond: IAllBondData }): ReactNode => {
  const { chainID }: { chainID: NetworkID } = useWeb3Context();
  if ((typeof bond.bondDiscount == "undefined") || !bond.isAvailable[NETWORK_CHAINID]) {
    return <Fragment>--</Fragment>;
  }
  return <Fragment>{bond.bondDiscount && trim(bond.bondDiscount * 100, 3)}%</Fragment>;
};
export default Bond;
